...
class MovieDetailsRepositoryShould {
  ...
  fun `return movie when api returns movie`() {
    every { movieApi.getMovieById(MOVIE_ID) } returns MOCKED_MOVIE_DTO

    val movie = underTest.getMovieWithId(MOVIE_ID)

    verify(exactly = 1) { movieApi.getMovieById(MOVIE_ID) }
    assertThat(movie, equalTo(Result.success(MOCKED_MOVIE)))
  }

  @Test
  fun `return failure with message when api returns null`() {
    every { movieApi.getMovieById(MOVIE_ID) } throws Throwable("Mocked error")
